...
contract TestDistributeHeldFee is TestBaseWorkflow {
  ...
  function testHeldFeeReimburse(
    ...
    evm.assume(feeDiscount <= snowLibraries().MAX_FEE());
    ...
    ISNOWFeeGauge feeGauge = ISNOWFeeGauge(address(69696969));
    ...
    uint256 discountedFee = fee - PRBMath.mulDiv(fee, feeDiscount, snowLibraries().MAX_FEE());
    ...
    uint256 heldFee = payAmountInWei -
      PRBMath.mulDiv(
        ...
        discountedFee + snowLibraries().MAX_FEE()
      ); // no discount
    uint256 balanceBefore = snowPaymentTerminalStore().balanceOf(_terminal, _projectId);
