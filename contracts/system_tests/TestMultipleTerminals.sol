...
contract TestMultipleTerminals is TestBaseWorkflow {
  ...
  SNOWERC20PaymentTerminal ERC20terminal;
  SNOWETHPaymentTerminal ETHterminal;
  ...
  function setUp() public override {
    ...
    _groupedSplits[0].splits.push(
      SNOWSplit({
        ...
        percent: snowLibraries().SPLITS_TOTAL_PERCENT(),
        projectId: 0,
        beneficiary: payable(caller),
        lockedUntil: 0,
        allocator: ISNOWSplitAllocator(address(0))
      })
    );
    ...
    evm.label(address(ERC20terminal), 'SNOWERC20PaymentTerminalUSD');

    ETHterminal = snowETHPaymentTerminal();
    ...
    _fundAccessConstraints.push(
      SNOWFundAccessConstraints({
        ...
        distributionLimitCurrency: snowLibraries().USD(),
        overflowAllowanceCurrency: snowLibraries().USD()
      })
    );
  ...
  function testMultipleTerminal() public {
    ...
    snowToken().approve(address(ERC20terminal), 20 * 10**18);
    evm.prank(caller); // back to regular msg.sender (bug?)
    ERC20terminal.pay(
      projectId,
      20 * 10**18,
    ...
    assertEq(
      ...
      PRBMath.mulDiv(
        5 * 10**18,
        snowLibraries().MAX_FEE(),
        snowLibraries().MAX_FEE() + ERC20terminal.fee()
      )
    );
    ...
    assertEq(
      ...
      initBalance +
        PRBMath.mulDiv(
          10 * 10**18,
          snowLibraries().MAX_FEE(),
          ETHterminal.fee() + snowLibraries().MAX_FEE()
        )
    );

    // redeem eth from the overflow by the token holder:
    uint256 totalSupply = snowController().totalOutstandingTokensOf(projectId, 5000);
    uint256 overflow = snowPaymentTerminalStore().currentTotalOverflowOf(projectId, 18, 1);
