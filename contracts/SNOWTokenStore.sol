...
import './interfaces/ISNOWTokenStore.sol';
...
*/
contract SNOWTokenStore is SNOWControllerUtility, SNOWOperatable, ISNOWTokenStore {
  //*********************************************************************//
  // --------------------------- custom errors ------------------------- //
  //*********************************************************************//
  ...
  */
  mapping(uint256 => ISNOWToken) public override tokenOf;
  ...
  */
  mapping(ISNOWToken => uint256) public override projectOf;
  ...
  */
  function totalSupplyOf(uint256 _projectId) external view override returns (uint256 totalSupply) {
    ...
    ISNOWToken _token = tokenOf[_projectId];

    // If the project has a current token, add it's total supply to the total.
    if (_token != ISNOWToken(address(0))) totalSupply = totalSupply + _token.totalSupply(_projectId);
  }
  ...
  */
    ...
    returns (uint256 balance)
  {
    ...
    if (_token != ISNOWToken(address(0))) balance = balance + _token.balanceOf(_holder, _projectId);
  }
  ...
  */
  ...
  ) SNOWOperatable(_operatorStore) SNOWControllerUtility(_directory) {
    projects = _projects;
  }
  ...
  */
  function issueFor(
  ...
  ) external override onlyController(_projectId) returns (ISNOWToken token) {
    ...
    if (tokenOf[_projectId] != ISNOWToken(address(0))) revert PROJECT_ALREADY_HAS_TOKEN();

    // Deploy the token contract.
    token = new SNOWToken(_name, _symbol);
  ...
  /**
    ...
    @dev
    This contract must have access to all of the token's `ISNOWToken` interface functions.
  ...
  */
  function changeFor(
    ...
    ISNOWToken _token,
    address _newOwner
  ) external override onlyController(_projectId) returns (ISNOWToken oldToken) {
    // Can't remove the project's token if the project requires claiming tokens.
    if (_token == ISNOWToken(address(0)) && requireClaimFor[_projectId])
    ...
    if (_token != ISNOWToken(address(0)) && _token.decimals() != 18)
  ...
  */
  function burnFrom(
    ...
    uint256 _claimedBalance = _token == ISNOWToken(address(0))
      ? 0
  ...
  */
  function claimFor(
    ...
    uint256 _projectId,
    uint256 _amount
  ) external override requirePermission(_holder, _projectId, SNOWOperations.CLAIM) {
    // Get a reference to the project's current token.
    ISNOWToken _token = tokenOf[_projectId];

    // The project must have a token contract attached.
    if (_token == ISNOWToken(address(0))) revert TOKEN_NOT_FOUND();
  ...
  */
  function transferFrom(
    ...
    address _recipient,
    uint256 _amount
  ) external override requirePermission(_holder, _projectId, SNOWOperations.TRANSFER) {
  ...
  */
  function shouldRequireClaimingFor(uint256 _projectId, bool _flag)
    external
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SNOWOperations.REQUIRE_CLAIM)
  {
    // Get a reference to the project's current token.
    ISNOWToken _token = tokenOf[_projectId];
