...
import './../interfaces/ISNOWPayoutRedemptionPaymentTerminal.sol';
...
import './SNOWOperatable.sol';
import './SNOWSingleTokenPaymentTerminal.sol';
...
/**
  ...
  @dev
  Adheres to -
  ISNOWPayoutRedemptionPaymentTerminal: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  SNOWSingleTokenPaymentTerminal: Generic terminal managing all inflows of funds into the protocol ecosystem for one token.
...
*/
abstract contract SNOWPayoutRedemptionPaymentTerminal is
  SNOWSingleTokenPaymentTerminal,
  SNOWOperatable,
  Ownable,
  ReentrancyGuard,
  ISNOWPayoutRedemptionPaymentTerminal
{
  ...
  */
  mapping(uint256 => SNOWFee[]) internal _heldFeesOf;
  ...
  */
  ISNOWSingleTokenPaymentTerminalStore public immutable override store;
  ...
  */
  ISNOWFeeGauge public override feeGauge;
  ...
  */
    ...
    returns (uint256)
  {
    ...
    uint256 _adjustedOverflow = (decimals == 18)
      ? _overflow
      : SNOWFixedPointNumber.adjustDecimals(_overflow, decimals, 18);
  ...
  */
  function heldFeesOf(uint256 _projectId) external view override returns (SNOWFee[] memory) {
  ...
  */
  function supportsInterface(bytes4 _interfaceId)
    ...
    view
    virtual
    override(SNOWSingleTokenPaymentTerminal, IERC165)
    returns (bool)
  {
    return
      _interfaceId == type(ISNOWPayoutRedemptionPaymentTerminal).interfaceId ||
      _interfaceId == type(ISNOWPayoutTerminal).interfaceId ||
      _interfaceId == type(ISNOWAllowanceTerminal).interfaceId ||
      _interfaceId == type(ISNOWRedemptionTerminal).interfaceId ||
  ...
  */
  constructor(
    ...
    ISNOWPrices _prices,
    ISNOWSingleTokenPaymentTerminalStore _store,
    address _owner
  )
    payable
    SNOWSingleTokenPaymentTerminal(_token, _decimals, _currency)
  ...
  */
  function redeemTokensOf(
    ...
    virtual
    override
    requirePermission(_holder, _projectId, SNOWOperations.REDEEM)
  ...
  */
  function useAllowanceOf(
    ...
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SNOWOperations.USE_ALLOWANCE)
  ...
  */
  function migrate(uint256 _projectId, ISNOWPaymentTerminal _to)
    external
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SNOWOperations.MIGRATE_TERMINAL)
  ...
  */
  function processFees(uint256 _projectId)
    ...
    requirePermissionAllowingOverride(
      ...
      _projectId,
      SNOWOperations.PROCESS_FEES,
      msg.sender == owner()
    )
  {
    // Get a reference to the project's held fees.
    SNOWFee[] memory _heldFees = _heldFeesOf[_projectId];
  ...
  */
  function setFeeGauge(ISNOWFeeGauge _feeGauge) external virtual override onlyOwner {
  ...
  */
  function _redeemTokensOf(
    ...
    {
      ISNOWRedemptionDelegate _delegate;
      ...
      if (_tokenCount > 0)
        ISNOWController(directory.controllerOf(_projectId)).burnTokensOf(
          _holder,
          _projectId,
          _tokenCount,
          '',
      ...
      if (_delegate != ISNOWRedemptionDelegate(address(0))) {
        SNOWDidRedeemData memory _data = SNOWDidRedeemData(
          _holder,
          _projectId,
          _fundingCycle.configuration,
          _tokenCount,
          SNOWTokenAmount(token, reclaimAmount, decimals, currency),
          _beneficiary,
          _memo,
          _metadata
        );
  ...
  */
  function _distributePayoutsOf(
    ...
    (SNOWFundingCycle memory _fundingCycle, uint256 _distributedAmount) = store.recordDistributionFor(
      _projectId,
      _amount,
      _currency
    );
    ...
    // Scoped section prevents stack too deep. `_feeDiscount`, `_feeEligibleDistributionAmount`, and `_leftoverDistributionAmount` only used within scope.
    {
      ...
      uint256 _feeDiscount = fee == 0 || isFeelessAddress[msg.sender]
        ? SNOWConstants.MAX_FEE_DISCOUNT
      ...
      // Take the fee.
      _fee = _feeDiscount == SNOWConstants.MAX_FEE_DISCOUNT || _feeEligibleDistributionAmount == 0
        ? 0
        : _takeFeeFrom(
          _projectId,
          _fundingCycle,
  ...
  */
  function _useAllowanceOf(
    ...
    (SNOWFundingCycle memory _fundingCycle, uint256 _distributedAmount) = store.recordUsedAllowanceOf(
      _projectId,
      _amount,
      _currency
    );
    ...
    // Scoped section prevents stack too deep. `_fee`, `_projectOwner`, `_feeDiscount`, and `_netAmount` only used within scope.
    {
      ...
      _fee = _feeDiscount == SNOWConstants.MAX_FEE_DISCOUNT
        ? 0
      ...
      if (_payoutAmount > 0) {
        ...
        if (_split.allocator != ISNOWSplitAllocator(address(0))) {
          ...
          if (isFeelessAddress[address(_split.allocator)])
            ...
            unchecked {
              _netPayoutAmount = _feeDiscount == SNOWConstants.MAX_FEE_DISCOUNT
                ? _payoutAmount
          ...
          if (_terminal == ISNOWPaymentTerminal(address(0))) revert TERMINAL_IN_SPLIT_ZERO_ADDRESS();
  ...
  */
  function _takeFeeFrom(
    ...
    if (_fundingCycle.shouldHoldFees()) {
      // Store the held fee.
      _heldFeesOf[_projectId].push(SNOWFee(_amount, uint32(fee), uint32(_feeDiscount), _beneficiary));
  ...
  */
  function _processFee(uint256 _amount, address _beneficiary) internal {
    ...
    ISNOWPaymentTerminal _terminal = directory.primaryTerminalOf(_PROTOCOL_PROJECT_ID, token);
  ...
  */
  function _pay(
    ...
    {
      ISNOWPayDelegate _delegate;
      uint256 _tokenCount;

      // Bundle the amount info into a SNOWTokenAmount struct.
      SNOWTokenAmount memory _bundledAmount = SNOWTokenAmount(token, _amount, decimals, currency);
      ...
      if (_tokenCount > 0)
        ...
        beneficiaryTokenCount = ISNOWController(directory.controllerOf(_projectId)).mintTokensOf(
          _projectId,
          _tokenCount,
          _beneficiary,
          '',
      ...
      // If a delegate was returned by the data source, issue a callback to it.
      if (_delegate != ISNOWPayDelegate(address(0))) {
        SNOWDidPayData memory _data = SNOWDidPayData(
          _payer,
          _projectId,
          _fundingCycle.configuration,
          _bundledAmount,
  ...
  */
  function _feeAmount(
    ...
    uint256 _discountedFee = _fee -
      PRBMath.mulDiv(_fee, _feeDiscount, SNOWConstants.MAX_FEE_DISCOUNT);

    // The amount of tokens from the `_amount` to pay as a fee.
    return
      _amount - PRBMath.mulDiv(_amount, SNOWConstants.MAX_FEE, _discountedFee + SNOWConstants.MAX_FEE);
  }
  ...
  */
  function _currentFeeDiscount(uint256 _projectId) internal view returns (uint256 feeDiscount) {
    ...
    if (directory.primaryTerminalOf(_PROTOCOL_PROJECT_ID, token) == ISNOWPaymentTerminal(address(0)))
      return SNOWConstants.MAX_FEE_DISCOUNT;

    // Get the fee discount.
    if (feeGauge == ISNOWFeeGauge(address(0)))
      feeDiscount = 0;
    ...
    if (feeDiscount > SNOWConstants.MAX_FEE_DISCOUNT) feeDiscount = 0;
  }
}
