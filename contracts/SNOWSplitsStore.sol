...
/**
  ...
  @dev
  Adheres to -
  ISNOWSplitsStore: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.
...
*/
contract SNOWSplitsStore is SNOWOperatable, ISNOWSplitsStore {
  //*********************************************************************//
  // --------------------------- custom errors ------------------------- //
  //*********************************************************************//
...
*/
  function splitsOf(
    ...
    uint256 _domain,
    uint256 _group
  ) external view override returns (SNOWSplit[] memory) {
  ...
  */
    ...
    )
  {
    ...
    for (uint256 _i = 0; _i < _groupedSplitsLength; ) {
      ...
      SNOWGroupedSplits memory _groupedSplit = _groupedSplits[_i];
  ...
  */
  function _set(
    ...
    SNOWSplit[] memory _currentSplits = _getStructsFor(_projectId, _domain, _group);
    ...
    for (uint256 _i = 0; _i < _splits.length; _i++) {
      ...
      if (_percentTotal > SNOWConstants.SPLITS_TOTAL_PERCENT) revert INVALID_TOTAL_PERCENT();
      ...
      if (_splits[_i].lockedUntil > 0 || _splits[_i].allocator != ISNOWSplitAllocator(address(0))) {
  ...
  */
  function _getStructsFor(
    ...
    uint256 _domain,
    uint256 _group
  ) private view returns (SNOWSplit[] memory) {
    ...
    SNOWSplit[] memory _splits = new SNOWSplit[](_splitCount);
    ...
    for (uint256 _i = 0; _i < _splitCount; _i++) {
      ...
      if (_packedSplitPart2 > 0) {
        ...
        // allocator in bits 48-207.
        _split.allocator = ISNOWSplitAllocator(address(uint160(_packedSplitPart2 >> 48)));
      }
