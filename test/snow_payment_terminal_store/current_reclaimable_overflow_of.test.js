...
describe('SNOWSingleTokenPaymentTerminalStore::currentReclaimableOverflowOf(...)', function () {
  const PROJECT_ID = 2;
  ...
  async function setup() {
    ...
    const mockJbTerminal2 = await deployMockContract(deployer, snowTerminal.abi);
  ...
  it('Should return claimable overflow from this terminal, with rate from active ballot', async function () {
    ...
    */
    ...
    expect(
      await SNOWSingleTokenPaymentTerminalStore[
  ...
  it('Should return 0 if claiming more than the total supply', async function () {
    const { SNOWSingleTokenPaymentTerminalStore } = await setup();
