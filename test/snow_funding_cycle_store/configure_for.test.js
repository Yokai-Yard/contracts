...
import isnowFundingCycleBallot from '../../artifacts/contracts/interfaces/ISNOWFundingCycleBallot.sol/ISNOWFundingCycleBallot.json';
import { BigNumber } from 'ethers';
import errors from '../helpers/errors.json';

describe('SNOWFundingCycleStore::configureFor(...)', function () {
  const PROJECT_ID = 2;
  ...
  async function setup() {
    ...
    const mockBallot = await deployMockContract(deployer, isnowFundingCycleBallot.abi);
    await mockBallot.mock.supportsInterface.returns(true);

    const snowFundingCycleStoreFactory = await ethers.getContractFactory('contracts/SNOWFundingCycleStore.sol:SNOWFundingCycleStore');
    const snowFundingCycleStore = await snowFundingCycleStoreFactory.deploy(mockJbDirectory.address);
  ...
  it('Should have no current or queued funding cycle before configuring', async function () {
    const { snowFundingCycleStore } = await setup();

    // Ballot status should be approved since there is no ballot.
    expect(await snowFundingCycleStore.currentBallotStateOf(PROJECT_ID)).to.eql(1);

    const [latestFundingCycle, ballotState] = await snowFundingCycleStore.latestConfiguredOf(
      PROJECT_ID,
    );
    expect(cleanFundingCycle(latestFundingCycle)).to.eql(EMPTY_FUNDING_CYCLE);
    expect(ballotState).to.deep.eql(1);
    expect(cleanFundingCycle(await snowFundingCycleStore.currentOf(PROJECT_ID))).to.eql(
      EMPTY_FUNDING_CYCLE,
    );
    expect(cleanFundingCycle(await snowFundingCycleStore.queuedOf(PROJECT_ID))).to.eql(
      EMPTY_FUNDING_CYCLE,
    );
  });

  it('Should create current funding cycle and queued cycle on first configure', async function () {
    const { controller, mockJbDirectory, snowFundingCycleStore } = await setup();
    ...
    const configureForTx = await snowFundingCycleStore
      .connect(controller)
      .configureFor(
        PROJECT_ID,
    ...
    await expect(configureForTx)
      .to.emit(snowFundingCycleStore, 'Configure')
      .withArgs(
    ...
    await expect(configureForTx)
      .to.emit(snowFundingCycleStore, `Init`)
    ...
    expect(
      cleanFundingCycle(await snowFundingCycleStore.get(PROJECT_ID, configurationTimestamp)),
    ).to.eql(expectedCurrentFundingCycle);

    let [latestFundingCycle, ballotState] = await snowFundingCycleStore.latestConfiguredOf(
      PROJECT_ID,
    );
    ...
    [latestFundingCycle, ballotState] = await snowFundingCycleStore.latestConfiguredOf(PROJECT_ID);
    expect(cleanFundingCycle(latestFundingCycle)).to.eql(expectedCurrentFundingCycle);
    expect(ballotState).to.deep.eql(1);
    expect(cleanFundingCycle(await snowFundingCycleStore.currentOf(PROJECT_ID))).to.eql(
      expectedCurrentFundingCycle,
  ...
  it('Should configure subsequent cycle during a funding cycle', async function () {
    const { controller, mockJbDirectory, snowFundingCycleStore, mockBallot } = await setup();
    await mockJbDirectory.mock.controllerOf.withArgs(PROJECT_ID).returns(controller.address);

    const firstFundingCycleData = createFundingCycleData();

    // Configure first funding cycle
    const firstConfigureForTx = await snowFundingCycleStore
      .connect(controller)
      .configureFor(
        PROJECT_ID,
    ...
    const secondConfigureForTx = await snowFundingCycleStore
      .connect(controller)
      .configureFor(
        PROJECT_ID,
    ...
    expect(
      cleanFundingCycle(await snowFundingCycleStore.get(PROJECT_ID, secondConfigurationTimestamp)),
  ...
  it('Should ignore failed configuration and roll over current configuration', async function () {
    const { controller, mockJbDirectory, mockBallot, snowFundingCycleStore } = await setup();
    await mockJbDirectory.mock.controllerOf.withArgs(PROJECT_ID).returns(controller.address);
    ...
    const failedConfigureForTx = await snowFundingCycleStore
      .connect(controller)
      .configureFor(
        PROJECT_ID,
  ...
  it('Should not use a funding cycle that fails a ballot', async function () {
    const { controller, mockJbDirectory, mockBallot, snowFundingCycleStore, addrs } = await setup();
    await mockJbDirectory.mock.controllerOf.withArgs(PROJECT_ID).returns(controller.address);
    ...
    expect(await snowFundingCycleStore.currentBallotStateOf(PROJECT_ID)).to.eql(2);
  ...
  it("Should hold off on using a reconfigured funding cycle if the current cycle's ballot duration doesn't end until after the current cycle is over", async function () {
    ...
    expect(await snowFundingCycleStore.currentBallotStateOf(PROJECT_ID)).to.eql(0);
  ...
  it('Should overwrite a pending reconfiguration', async function () {
    ...
    // Configure second funding cycle
    const thirdConfigureForTx = await snowFundingCycleStore
      .connect(controller)
      .configureFor(
        PROJECT_ID,
  ...
  it('Should configure subsequent cycle with a weight derived from previous cycle if a value of 0 is passed in', async function () {
    const { controller, mockJbDirectory, snowFundingCycleStore, addrs } = await setup();
    await mockJbDirectory.mock.controllerOf.withArgs(PROJECT_ID).returns(controller.address);
  ...
  it('Should configure subsequent cycle during a rolled over funding cycle overriding an already-proposed configuration', async function () {
    ...
    expect(
      cleanFundingCycle(await snowFundingCycleStore.get(PROJECT_ID, thirdConfigurationTimestamp)),
  ...
  it(`Can't configure if ballot is not IERC165 compliant`, async function () {
    const { controller, deployer, mockJbDirectory, snowFundingCycleStore } = await setup();
    await mockJbDirectory.mock.controllerOf.withArgs(PROJECT_ID).returns(controller.address);
    
    const mockBallotNew = await deployMockContract(deployer, isnowFundingCycleBallot.abi);
