...
/**
 * Deploy a test SNOWToken contract
 * @param {string} name
 * @param {string} symbol
 * @return {ethers.Contract}
 */
export async function deployJbToken(name, symbol) {
  const snowTokenFactory = await ethers.getContractFactory('SNOWToken');
  return await snowTokenFactory.deploy(name, symbol);
}
 ...
 * @summary Should mirror the bit logic in SNOWFundingCycleMetadataResolver.sol.
...
/**
 * Returns an array of SNOWSplits
 * @param {custom obj} count being the number of splits in the returned array, rest of the
 * object is a SNOWSplit
 * @return a SNOWSplit array of count objects
 */
 ...
 * @summary Should create a struct based on the definition in structs/SNOWFundingCycleData.sol.
