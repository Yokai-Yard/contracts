...
describe('SNOWETHERC20SplitsPayer::addToBalanceOf(...)', function () {
  ...
  it(`Should send ETH towards allocator if set in split and emit event`, async function () {
    ...
    await expect(tx).to.emit(snowSplitsPayer, 'AddToBalance').withArgs(
      PROJECT_ID,
      DEFAULT_BENEFICIARY,
      ethToken,
      AMOUNT,
  ...
  it(`Should send fund directly to the caller, if no allocator, project ID, beneficiary or default beneficiary is set,  and emit event`, async function () {
    ...
    await expect(tx).to.emit(snowSplitsPayerWithoutDefaultBeneficiary, 'AddToBalance').withArgs(
      PROJECT_ID,
      caller.address,
      ethToken,
      AMOUNT,
  ...
  it(`Should send erc20 leftover to project id if set and emit event`, async function () {
    ...
    await expect(
      ...
      .to.emit(snowSplitsPayer, 'AddToBalance')
      .withArgs(
        PROJECT_ID,
        DEFAULT_BENEFICIARY,
        mockToken.address,
